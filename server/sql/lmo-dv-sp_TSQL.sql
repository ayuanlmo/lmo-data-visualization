...
CREATE PROCEDURE [dbo].[usp_InsertTemplateItem]
	...
	AS
BEGIN
		INSERT INTO [Templates] ( [id], [name], [description], [path], [cover], [gifCover], [createTime], [type],[index] )
		VALUES
			( @Id, @Name, @Description, @Path, @Cover, @GifCover, @CreateTime, @Type ,@Index);
END;
GO
...
UPDATE [Templates]
SET
    [name] = @Name, [description] = @Description, [path] = @Path, [cover] = @Cover, [gifCover] = @GifCover, createTime = @CreateTime, [type] = @Type
    [index] = @Index
WHERE
    [id] = @Id;
END;
...
UPDATE [Colors]
SET
    [value] = @value, [cssCode] = @cssCode, [type] = @type
WHERE id = @id;
END;
GO
...
UPDATE [Resources]
SET
    [name] = @name, [template] = @template, [filePath] = @filePath, [createTime] = @createTime, [templatePath] = @templatePath, [url] = @url, [gifPath] = @gifPath, [videoCover] = @videoCover, [clarity] = @clarity, [status] = @status, [taskConfig] = @taskConfig
WHERE [id] = @id;
END;
GO
...
UPDATE [UpLoadFiles]
SET
    [name] = @name, [path] = @path, [cover] = @cover, [createTime] = @createTime, [type] = @type, [hash] = @hash
WHERE id = @id;
END;
GO
